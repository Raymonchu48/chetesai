...
import GluteExerciseVisual from "@/components/exercises/GluteExerciseVisual";
...
export default function ExerciseMediaVisual({ item }: { item: ExerciseMediaItem }) {
  ...
  if (code.startsWith("CHE-GLU-") || code === "CHE-PIE-005") return <GluteExerciseVisual code={code} name={item.nombre} />;
  ...
  if (uploaded) return <img src={uploaded} alt={item.nombre} className="h-full w-full object-cover" />;

  return <div className="flex h-full min-h-52 flex-col items-center justify-center bg-gradient-to-br from-emerald-950 via-emerald-900 to-slate-950 px-6 text-center text-white"><Dumbbell className="mb-4 h-12 w-12"/><p className="text-xs font-semibold uppercase tracking-[.24em] text-emerald-200">Chetesaí Fitness+</p><p className="mt-2 text-xl font-bold">{item.nombre}</p><p className="mt-2 text-sm text-white/70">Visual específico en preparación</p></div>;
}
