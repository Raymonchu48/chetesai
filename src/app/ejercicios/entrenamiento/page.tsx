    ...
    <AppSidebar>
      <div className="min-h-screen bg-[#f7f9f6] text-slate-950">
        <div className="mx-auto max-w-[1800px] p-4 md:p-6">
          ...
          <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_430px]">
              ...
              {loading ? <div className="rounded-2xl border bg-white py-24 text-center text-slate-500">Cargando biblioteca visual...</div> : visibleItems.length ? (
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {visibleItems.map((item) => (
                    <ExerciseCard key={item._id} item={item} selected={selected?._id === item._id} favorite={favorites.includes(item._id)} onSelect={() => selectExercise(item)} onFavorite={() => toggleFavorite(item._id)}/>
                  ))}
                </div>
            ...
            <aside className="sticky top-5 hidden max-h-[calc(100vh-2.5rem)] overflow-y-auto rounded-2xl border border-slate-200 bg-white shadow-lg xl:block">
              {selected ? <ExerciseDetail item={selected} favorite={favorites.includes(selected._id)} onFavorite={() => toggleFavorite(selected._id)} onImageUploaded={updateExerciseImage}/> : <div className="p-10 text-center text-slate-500">Selecciona un ejercicio para ver su ficha.</div>}
            </aside>
          </div>
        ...
        <Dialog open={mobileDetailOpen} onOpenChange={setMobileDetailOpen}>
          <DialogContent className="max-h-[94vh] max-w-3xl overflow-y-auto p-0 xl:hidden">{selected ? <ExerciseDetail item={selected} favorite={favorites.includes(selected._id)} onFavorite={() => toggleFavorite(selected._id)} onImageUploaded={updateExerciseImage}/> : null}</DialogContent>
        </Dialog>
      </div>
    </AppSidebar>
  );
...
function ExerciseCard({ item, selected, favorite, onSelect, onFavorite }: { item: Ejercicio; selected: boolean; favorite: boolean; onSelect: () => void; onFavorite: () => void }) {
  return (
    <article role="button" tabIndex={0} onClick={onSelect} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onSelect(); }} className={`group cursor-pointer overflow-hidden rounded-2xl border bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg ${selected ? "border-[#6bb300] ring-1 ring-[#6bb300]" : "border-slate-200"}`}>
      ...
      <div className="h-48 overflow-hidden bg-white p-2"><MediaHero item={item}/></div>
...
function ExerciseDetail({ item, favorite, onFavorite, onImageUploaded }: { item: Ejercicio; favorite: boolean; onFavorite: () => void; onImageUploaded: (url: string) => void }) {
  ...
  return (
    <div>
      ...
      <div className="mx-5 h-64 overflow-hidden rounded-xl border border-slate-100 bg-[#fbfaf6] p-2"><MediaHero item={item}/></div>
