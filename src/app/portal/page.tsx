    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_right,_rgba(140,219,120,0.13),_transparent_30%),linear-gradient(180deg,#f4f1e9_0%,#faf9f5_52%,#f1eee7_100%)] px-4 py-6 text-[#202724] sm:px-6 sm:py-10">
      <div className="mx-auto max-w-6xl">
        <header className="relative mb-7 overflow-hidden rounded-[30px] border border-[#d7b86b]/30 bg-[#111a15] p-6 text-white shadow-[0_22px_70px_rgba(17,26,21,0.22)] sm:p-8">
          <div className="absolute -right-16 -top-20 h-64 w-64 rounded-full bg-[#8cdb78]/15 blur-3xl" />
          <div className="relative flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[11px] font-black uppercase tracking-[0.24em] text-[#d7b86b]">Chetesaí Fitness+</p><h1 className="mt-3 flex items-center gap-3 text-3xl font-black tracking-tight sm:text-4xl"><span className="grid h-12 w-12 shrink-0 place-items-center overflow-hidden rounded-2xl bg-[#8cdb78] p-1.5"><img src="/brand/chetesai-logo-mark.svg" alt="Logotipo de Chetesaí Fitness+" className="h-full w-full object-contain" /></span>Mi entrenamiento</h1><p className="mt-4 max-w-xl text-sm leading-6 text-white/65">{data?.cliente?.nombre ? `Hola, ${data.cliente.nombre}. Aquí tienes tu planificación activa.` : "Tu espacio privado de entrenamiento."}</p></div>
          <div className="w-full rounded-xl border border-[#8cdb78]/25 bg-[#8cdb78]/10 p-1 text-[#9fe68f] sm:w-52"><LogoutButton /></div></div>
        </header>
        ...
        {loading ? <p className="py-16 text-center text-[#707872]">Cargando tu entrenamiento...</p> : error ? <Card><CardContent className="py-12 text-center text-red-600">{error}</CardContent></Card> : !data?.asignacion || !data.rutina ? (
          ...
          <>
            <section className="mb-6 rounded-[26px] border border-[#ded8cd] bg-[#fffdf9]/95 p-6 shadow-sm backdrop-blur"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#46624f]">Plan activo</p><h2 className="mt-2 text-2xl font-bold">{data.rutina.nombre}</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-[#707872]">{data.rutina.descripcion}</p></div><div className="flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-[#e9f2ea] px-3 py-1 text-[#46624f]">{data.rutina.dias_semana} días/semana</span><span className="rounded-full bg-[#f0ede7] px-3 py-1">{data.rutina.nivel}</span></div></div></section>

            <div className="mb-6 flex gap-2 overflow-x-auto pb-2">{Array.from({ length: routineDays }, (_, index) => index + 1).map((day) => <Button key={day} onClick={() => setSelectedDay(day)} className={selectedDay === day ? "bg-[#2f9e24] text-white shadow-lg shadow-[#2f9e24]/15 hover:bg-[#27891e]" : "border border-[#ded8cd] bg-[#fffdf9] text-[#29312e] hover:border-[#8cdb78] hover:bg-[#eaf5e8]"}>Día {day} · {days[day - 1]}</Button>)}</div>
