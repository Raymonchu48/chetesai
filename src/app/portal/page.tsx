...
import { Activity, CheckCircle2, ChevronDown, CircleCheck, CircleX, Clock3, Dumbbell, History, Lightbulb, PlayCircle, RotateCcw, SkipForward, Square, Target, TrendingUp, TriangleAlert } from "lucide-react";
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_right,_rgba(140,219,120,0.13),_transparent_30%),linear-gradient(180deg,#f4f1e9_0%,#faf9f5_52%,#f1eee7_100%)] px-4 py-6 text-[#202724] sm:px-6 sm:py-10">
      <div className="mx-auto max-w-6xl">
        ...
        {loading ? <p className="py-16 text-center text-[#707872]">Cargando tu entrenamiento...</p> : error ? <Card><CardContent className="py-12 text-center text-red-600">{error}</CardContent></Card> : !data?.asignacion || !data.rutina ? (
          ...
          <>
            <section className="mb-6 rounded-[26px] border border-[#ded8cd] bg-[#fffdf9]/95 p-6 shadow-sm backdrop-blur"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#46624f]">Plan activo</p><h2 className="mt-2 text-2xl font-bold">{data.rutina.nombre}</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-[#707872]">{data.rutina.descripcion}</p></div><div className="flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-[#e9f2ea] px-3 py-1 text-[#46624f]">{trainingDays.length} {trainingDays.length === 1 ? "día" : "días"}/semana</span><span className="rounded-full bg-[#f0ede7] px-3 py-1">{data.rutina.nivel}</span></div></div></section>
