  ...
  try {
    ...
    const ejercicios = rutinaId
      ? await serviceRest<Array<Record<string, unknown>>>(
          `rutina_ejercicios?rutina_id=eq.${encodeURIComponent(rutinaId)}&visible_cliente=eq.true&select=*,ejercicios(id,nombre,grupo_muscular,material,imagen_url,miniatura_url,gif_url,video_url,descripcion,tecnica,consejos)&order=dia.asc,orden.asc`
        )
      : [];
