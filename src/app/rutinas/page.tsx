...
import { Copy, Dumbbell, Pencil, Plus, Search, Trash2 } from "lucide-react";
...
export default function RutinasPage() {
  ...
  const visible = useMemo(() => {
    ...
    return rutinas.filter((item) => {
      const matchesText = !term || [item.nombre, item.descripcion, labels[item.objetivo]]
        .filter(Boolean)
      ...
      <div className="mx-auto max-w-7xl p-6 md:p-8">
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="flex items-center gap-3 text-3xl font-bold tracking-tight">
              <Dumbbell className="h-8 w-8 text-primary" /> Rutinas
            </h1>
            <p className="mt-1 text-muted-foreground">{rutinas.length} rutinas registradas</p>
          </div>
        ...
        <div className="mb-6 grid gap-3 md:grid-cols-3">
          <div className="relative md:col-span-2">
            ...
            <Input className="pl-9" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar rutina..." />
          </div>
          <Select value={objectiveFilter} onValueChange={setObjectiveFilter}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="todos">Todos los objetivos</SelectItem>
              {objetivos.map((item) => <SelectItem key={item} value={item}>{labels[item]}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
                ...
                <CardContent className="p-5">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{labels[item.objetivo]} · {labels[item.nivel]}</p>
                    </div>
                    <div className="flex">
                      <Button variant="ghost" size="icon" onClick={() => openEdit(item)}><Pencil className="h-4 w-4" /></Button>
                      <Button variant="ghost" size="icon" onClick={() => duplicate(item._id)}><Copy className="h-4 w-4" /></Button>
                      <Button variant="ghost" size="icon" className="text-destructive" onClick={() => deactivate(item._id)}><Trash2 className="h-4 w-4" /></Button>
                    </div>
                  </div>
                  <div className="mt-4 flex flex-wrap gap-2 text-xs">
                    <span className="rounded-full bg-primary/10 px-2.5 py-1 text-primary">{item.dias_semana} días/semana</span>
                    {item.duracion_semanas ? <span className="rounded-full bg-muted px-2.5 py-1">{item.duracion_semanas} semanas</span> : null}
                    {item.duracion_sesion_minutos ? <span className="rounded-full bg-muted px-2.5 py-1">{item.duracion_sesion_minutos} min</span> : null}
                  </div>
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent className="max-w-2xl">
            <DialogHeader><DialogTitle>{editingId ? "Editar rutina" : "Nueva rutina"}</DialogTitle></DialogHeader>
            <div className="grid gap-4 py-2 md:grid-cols-2">
              <Field label="Nombre *"><Input value={form.nombre} onChange={(e) => setForm({ ...form, nombre: e.target.value })} /></Field>
              <Field label="Objetivo">
                <Select value={form.objetivo} onValueChange={(value) => setForm({ ...form, objetivo: value })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{objetivos.map((item) => <SelectItem key={item} value={item}>{labels[item]}</SelectItem>)}</SelectContent></Select>
              </Field>
              <Field label="Nivel">
                <Select value={form.nivel} onValueChange={(value) => setForm({ ...form, nivel: value })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{niveles.map((item) => <SelectItem key={item} value={item}>{labels[item]}</SelectItem>)}</SelectContent></Select>
              </Field>
            ...
            <Button className="w-full" onClick={save}>{editingId ? "Guardar cambios" : "Crear rutina"}</Button>
          </DialogContent>
        </Dialog>
      </div>
    </AppSidebar>
